...
# models are tested on identical scenarios. Writes results to eval_results_act.json.
import json
import numpy as np
...
print(f"[eval_act] loading policy from hub: {REPO_ID}")
...
print("[eval_act] policy loaded, device=", device)
...
print(f"[eval_act] starting {len(CUBE_XY_LIST)} episodes")
for ep, (cx, cy) in enumerate(CUBE_XY_LIST):
    ...
    print(f"[eval_act] ep {ep:2d} cube=({cx:.3f},{cy:.3f}) success={success} steps={final_step} "
          f"max_lift={max_lift:.3f} min_xy_err={min_xy_err:.3f} grasp_attempted={grasp_attempted}")

n_success = sum(r["success"] for r in results)
summary = {
    "model": "ACT (hub, jamongsteak/act_pickplace_model)",
...
with open("eval_results_act.json", "w") as f:
    json.dump(summary, f, indent=2)

print(f"[eval_act] DONE success_rate={summary['success_rate']:.2f} ({n_success}/{len(results)}) "
