...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        dataset_path = os.path.join(self.dataset_dir, f'episode_{episode_id}.npz')
        with np.load(dataset_path) as root:
            is_sim = True
            ...
            for cam_name in self.camera_names:
                image_dict[cam_name] = root[f'image_{cam_name}'][start_ts]
...
def get_norm_stats(dataset_dir, num_episodes):
    ...
    for episode_idx in range(num_episodes):
        dataset_path = os.path.join(dataset_dir, f'episode_{episode_idx}.npz')
        with np.load(dataset_path) as root:
            qpos = root['qpos'][()]
            action = root['action'][()]
