...
# focal: 초점거리. 크면 화각이 좁아져 줌인된다. 보는 폭 = 거리 x 20.955 / focal.
# 카메라를 (2.0,0,2.0)으로 옮겨 작업영역까지 1.48m -> 2.55m가 됐으므로, 기존
# 화각을 유지하는 것만으로도 focal 24 -> 41mm가 필요하다(그 위가 실제 줌인).
#   41mm -> 큐브 12px | 60mm -> 18px | 80mm -> 24px | 97mm 이상은 목표 마커가 잘림
# 60mm를 쓴다. 큐브를 키우는 것보다 "팔과 그리퍼가 화면에 함께 보이는 것"이
# 중요하다 — 손이 큐브 대비 어디 있는지 보여야 폐루프 보정이 된다. 참고 사례
# (Deepkar/redblock-pick-place-50ep)도 망원이 아니라 넓은 화각 + 고해상도다.
# 60mm 커버리지는 0.87 x 0.85m로, 로봇 베이스부터 목표 마커까지 다 들어온다.
OVER_FOCAL  = 60.0
WRIST_FOCAL = 16.0   # 손목캠은 그대로. 이미 큐브를 18~31px로 보고 있다.

# === GUI에서 조정할 값 =======================================================
# 사용자가 GUI에서 잡은 값. 광축이 지면과 만나는 점은 (0.32, 0)으로, 정면에서
# 50도 내려보는 구도다. 팔이 큐브 뒤쪽에서 접근하므로 천장 수직 뷰보다 가림이
# 훨씬 적다 (가림 여부는 cam_check_current.py로 확인할 것).
